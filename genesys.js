...
// Cache token in memory (persists across warm invocations)
...
async function getAccessToken() {
  ...
  const res = await fetch(loginUrl, {
    ...
    headers: {
      ...
      Authorization:
        "Basic " +
        Buffer.from(`${GENESYS_CLIENT_ID}:${GENESYS_CLIENT_SECRET}`).toString(
          "base64"
        ),
    },
...
async function genesysAPI(path, body, token) {
  ...
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Genesys API ${path} failed (${res.status}): ${err}`);
  }
  return res.json();
}

// Get all users (agents) — paginated
async function getAllUsers(token) {
  ...
  while (hasMore) {
    const data = await genesysAPI(
      `/api/v2/users?pageSize=100&pageNumber=${page}&expand=presence`,
      null,
      token
    );
...
// Get conversation aggregates (handle time, talk, hold, acw, transfers)
async function getConversationAggregates(token, interval, userIds) {
  const body = {
    interval,
    granularity: "PT720H", // Single bucket for the whole period
    groupBy: ["userId"],
    metrics: [
      "nOffered",
      "nConnected",
  ...
  // If we have specific user IDs, filter by them
...
// Get quality evaluations aggregates
async function getEvaluationAggregates(token, interval) {
  // Use the evaluations query endpoint
  try {
    const data = await genesysAPI(
      `/api/v2/quality/evaluations/query?pageSize=100&expandAnswerTotalScores=true&startTime=${encodeURIComponent(interval.split("/")[0])}&endTime=${encodeURIComponent(interval.split("/")[1])}`,
      null,
      token
    );
    return data;
  } catch (e) {
    console.warn("Evaluation query failed:", e.message);
    return null;
  }
}

// Get agent evaluations scores
...
// Build the interval string for a given month
...
function extractStats(results) {
  ...
  for (const group of results.results) {
    ...
    const handleCount = stats.tHandle?.count || 1;
    const talkCount = stats.tTalk?.count || 1;
    const holdCount = stats.tHeld?.count || 1;
    const acwCount = stats.tAcw?.count || 1;
    const transfers = stats.nTransferred?.count || 0;

    agents[userId] = {
      answered,
      handled: handleCount,
      avgHandle: handleSum / handleCount / 1000, // ms → seconds
      avgTalk: talkSum / talkCount / 1000,
      avgHold: holdSum / holdCount / 1000,
      avgAcw: acwSum / acwCount / 1000,
      totalHandle: handleSum / 1000,
      totalTalk: talkSum / 1000,
      totalHold: holdSum / 1000,
      totalAcw: acwSum / 1000,
      transfers,
    };
  }
  ...
  try {
    if (!GENESYS_CLIENT_ID || !GENESYS_CLIENT_SECRET) {
      return res.status(500).json({
        error: "Genesys credentials not configured",
        setup:
          "Set GENESYS_CLIENT_ID and GENESYS_CLIENT_SECRET in Vercel environment variables",
      });
    }
    ...
    // ─── Action: Get KPIs for a month ───
    if (action === "kpis") {
      const month = req.query.month || "2026-02";
      const interval = monthInterval(month);

      // Fetch users + conversation aggregates in parallel
      const [users, convAgg] = await Promise.all([
        getAllUsers(token),
      ...
      for (const user of users) {
        ...
        if (!conv || conv.handled === 0) continue;

        kpis[user.name] = {
          genesysId: user.id,
          answered: conv.answered,
          handled: conv.handled,
          avgHandle: Math.round(conv.avgHandle), // seconds
          avgTalk: Math.round(conv.avgTalk),
          avgHold: Math.round(conv.avgHold),
          avgAcw: Math.round(conv.avgAcw),
          transfers: conv.transfers,
          // Map to Command Hub KPI format
          aht: Math.round(conv.avgHandle), // AHT in seconds
          interactions: conv.handled,
        };
      }
      ...
      return res.json({
        ...
        agentCount: Object.keys(kpis).length,
        kpis,
      });
    }

    // ─── Action: Get real-time agent status ───
    if (action === "status") {
      const users = await getAllUsers(token);
      const statuses = users.map((u) => ({
        ...
        name: u.name,
        presence:
          u.presence?.presenceDefinition?.systemPresence || "Unknown",
      }));
    ...
    return res.json({
      ok: true,
      message: "Genesys Cloud KPI Proxy",
      region: GENESYS_REGION,
      actions: {
        health: "GET /api/genesys?action=health",
        users: "GET /api/genesys?action=users",
        kpis: "GET /api/genesys?action=kpis&month=2026-02",
        status: "GET /api/genesys?action=status",
      },
    });
  } catch (err) {
